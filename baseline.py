...
hostlist = [
    ["192.168.100.128", "root", "toor"]
]

for x in hostlist:
    # Base_Line(ip,username,password,port=22)
    Base_Line(x[0], x[1], x[2])
...
#    def run(self):
#         field = {}
#         resrow = self.run_conmand("cat /etc/pam.d/system-auth | grep pam_cracklib.so")
#         currow = resrow
...
#             if col.__len__() == 2:
#                 field[col[0]] = col[1]
#         for key in pass_safe:
#             if field.has_key(key):
#                 if abs(int(field[key])) >= abs(pass_safe[key][0]):
#                     LOG.info(u"合格 " + pass_safe[key][1] + ":" + str(field[key]))
#                 else:
#                     LOG.info(u"不合格 " + pass_safe[key][1] + ":" + str(field[key]))
#                     currow = currow.replace(key + "=" + field[key], key + "=" + str(pass_safe[key][0]))
#                     self.run_conmand("sed -i \"/" + resrow + "/c " + currow + "\" /etc/pam.d/system-auth")
#                     resrow = currow
#                     LOG.info(u"修改   "  + str(pass_safe[key][0]))
#             else:
#                 LOG.info(u"不合格 " + pass_safe[key][1] + u":无")
#                 currow += " " + key + "=" + str(pass_safe[key][0])
#                 self.run_conmand("sed -i \"/" + resrow + "/c " + currow + "\" /etc/pam.d/system-auth")
#                 resrow = currow
#                 LOG.info(u"修改   " + str(pass_safe[key][0]))
#         LOG.debug(currow)
# ######
#         resrow = self.run_conmand("cat /etc/ssh/sshd_config | grep ^PermitRootLogin")
#         if resrow:
#             resrow = resrow[0].strip()
#             if resrow[16:] == user_safe["PermitRootLogin"][0]:
#                 LOG.info(u"合格 " + user_safe["PermitRootLogin"][1] + ":" + resrow[16:])
#             else:
#                 LOG.info(u"不合格 " + user_safe["PermitRootLogin"][1] + ":" + resrow[16:])
#                 currow = resrow[:16] + user_safe["PermitRootLogin"][0]
#                 self.run_conmand("sed -i \"/" + resrow + "/c " + currow + "\" /etc/ssh/sshd_config")
#                 LOG.info(u"修改   " + user_safe["PermitRootLogin"][1] + ":" + user_safe["PermitRootLogin"][0])
#         else:
#             resrow = self.run_conmand("cat /etc/ssh/sshd_config | grep ^#PermitRootLogin")
#             if resrow:
#                 resrow = resrow[0].strip()
#                 currow = resrow[:16]
#                 currow += "no"
#                 LOG.info(u"不合格 " + user_safe["PermitRootLogin"][1] + ":" + u"无")
#                 self.run_conmand("sed -i \"/" + resrow + "/c " + currow + "\" /etc/ssh/sshd_config")
#                 LOG.info(u"修改   " + user_safe["PermitRootLogin"][1] + ":" + user_safe["PermitRootLogin"][0])
#             else:
#                 LOG.info(u"不合格 " + user_safe["PermitRootLogin"][1] + ":" + u"无")
#                 self.run_conmand("echo \"PermitRootLogin no\" >> /etc/ssh/sshd_config")
#                 LOG.info(u"修改   " + user_safe["PermitRootLogin"][1] + ":" + user_safe["PermitRootLogin"][0])

#     def safe_umask():
#         umaskfile = ["/etc/profile", "/etc/csh.login", "/etc/csh.cshrc", "/etc/bashrc"]
#         # umask 027

        # if field.has_key("minlen"):
        #     if field["minlen"] < pass_safe["minlen"]:
        #         LOG.info(u"不合格 最小密码长度为:" + str(field["minlen"]))
        #         field["minlen"] = pass_safe["minlen"]
        #         LOG.info(u"修改   "  + str(field["minlen"]))
        # else:
        #     LOG.info(u"不合格 最小密码长度:无")
        #     field["minlen"] = pass_safe["minlen"]
        #     LOG.info(u"修改   :"  + str(field["minlen"]))

        # if field.has_key("ucredit"):
        #     if field["ucredit"] > pass_safe["ucredit"]:
        #         LOG.info(u"不合格 密码包含大写字母至少:" + str(field["ucredit"]))
        #         field["ucredit"] = pass_safe["ucredit"]
        #         LOG.info(u"修改   "  + str(field["ucredit"]))
        # else:
        #     LOG.info(u"不合格 密码包含大写字母:无")
        #     field["ucredit"] = pass_safe["ucredit"]
        #     LOG.info(u"修改   :"  + str(field["ucredit"]))

        # if field.has_key("lcredit"):
        #     if field["lcredit"] > pass_safe["lcredit"]:
        #         LOG.info(u"不合格 密码包含小写字母:" + str(field["lcredit"]))
        #         field["lcredit"] = pass_safe["lcredit"]
        #         LOG.info(u"修改   "  + str(field["lcredit"]))
        # else:
        #     LOG.info(u"不合格 密码包含小写字母:无")
        #     field["lcredit"] = pass_safe["lcredit"]
        #     LOG.info(u"修改   :"  + str(field["lcredit"]))

        # if field.has_key("dcredit"):
        #     if field["dcredit"] > pass_safe["dcredit"]:
        #         LOG.info(u"不合格 密码包含数字至少:" + str(field["dcredit"]))
        #         field["dcredit"] = pass_safe["dcredit"]
        #         LOG.info(u"修改   "  + str(field["dcredit"]))
        # else:
        #     LOG.info(u"不合格 密码包含数字:无")
        #     field["dcredit"] = pass_safe["dcredit"]
        #     LOG.info(u"修改   :"  + str(field["dcredit"]))

        # if field.has_key("ocredit"):
        #     if field["ocredit"] > pass_safe["ocredit"]:
        #         LOG.info(u"不合格 密码包含字符至少:" + str(field["ocredit"]))
        #         field["ocredit"] = pass_safe["ocredit"]
        #         LOG.info(u"修改   "  + str(field["ocredit"]))
        # else:
        #     LOG.info(u"不合格 密码包含字符:无")
        #     field["ocredit"] = pass_safe["ocredit"]
        #     LOG.info(u"修改   :"  + str(field["ocredit"]))

        # if field.has_key("retry"):
        #     if field["retry"] > pass_safe["retry"]:
        #         LOG.info(u"不合格 密码尝试次数:" + str(field["retry"]))
        #         field["retry"] = pass_safe["retry"]
        #         LOG.info(u"修改   "  + str(field["retry"]))
        # else:
        #     LOG.info(u"不合格 密码尝试次数:无")
        #     field["retry"] = pass_safe["retry"]
        #     LOG.info(u"修改   :"  + str(field["retry"]))